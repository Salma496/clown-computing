...
const ALLOWED_NEXT = { 'To Do': 'In Progress', 'In Progress': 'In Review', 'In Review': 'Done' };
const PRIORITY_DOT = { High: 'bg-red-400', Medium: 'bg-yellow-400', Low: 'bg-green-400' };
const COLUMN_ACCENT = { 'To Do': 'border-zinc-600', 'In Progress': 'border-blue-500', 'In Review': 'border-purple-500', 'Done': 'border-green-500' };
const COLUMN_HEADER_COLOR = { 'To Do': 'text-zinc-400', 'In Progress': 'text-blue-400', 'In Review': 'text-purple-400', 'Done': 'text-green-400' };

function TaskCard({ task, onClick, users }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: task.taskId, data: { task },
  });
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} onClick={onClick}
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <h3 className="text-white text-sm font-semibold leading-snug group-hover:text-[#f97316] transition-colors line-clamp-2">{task.title}</h3>
        <div className={`w-2 h-2 rounded-full flex-shrink-0 mt-1 ${PRIORITY_DOT[task.priority] || 'bg-zinc-500'}`} title={task.priority} />
      </div>
      {task.description && <p className="text-zinc-500 text-xs line-clamp-2 mb-3">{task.description}</p>}
...
export default function Dashboard() {
  ...
  const handleDragEnd = async ({ active, over }) => {
    ...
    if (ALLOWED_NEXT[draggedTask.status] !== targetStatus) {
      toast.error(`Can only move forward: ${draggedTask.status} → ${ALLOWED_NEXT[draggedTask.status]}`);
      return;
    }
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
        ...
        <div className="max-w-screen-2xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <nav className="flex items-center gap-1">
              <button className="text-white text-sm font-semibold px-3 py-1.5 rounded-lg bg-white/8">Board</button>
              <button onClick={() => setShowProjects(true)} className="text-zinc-500 hover:text-white text-sm px-3 py-1.5 rounded-lg hover:bg-white/5 transition-all">Projects</button>
            </nav>
          </div>
          <div className="flex items-center gap-4">
                ...
                return (
                  <div key={col} className={`bg-[#111118] border-t-2 ${COLUMN_ACCENT[col]} rounded-xl flex flex-col min-h-[500px]`}>
                    <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
                      <h2 className={`text-xs font-black uppercase tracking-widest font-['Syne',sans-serif] ${COLUMN_HEADER_COLOR[col]}`}>{col}</h2>
                      <span className="text-zinc-600 text-xs bg-white/5 px-2 py-0.5 rounded-full">{colTasks.length}</span>
                    </div>
                    <div className="flex-1 p-3">
                      <SortableContext items={colTasks.map(t => t.taskId)} strategy={verticalListSortingStrategy} id={col}>
                        <div className="space-y-2">
                          {colTasks.length === 0 ? (
                            ...
                            colTasks.map(task => (
                              <TaskCard key={task.taskId} task={task} users={users} onClick={() => setSelectedTask(task)} />
                            ))
                          )}
                        </div>
                      </SortableContext>
      ...
      {selectedTask && (
        <TaskModal
          ...
          onDeleted={() => { fetchAll(); setSelectedTask(null); }}
        />
      )}
      {showCreate && (
        <CreateTaskModal users={users} teams={teams} onClose={() => setShowCreate(false)} onCreated={fetchAll} />
      )}
    </div>
  );
}
